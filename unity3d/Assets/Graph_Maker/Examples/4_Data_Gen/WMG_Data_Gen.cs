...
public class WMG_Data_Gen : MonoBehaviour {
	...
	private bool isAutomaticAnimating = false;
...
//	private string realTimePropName = "RelativePosition";
	...
	void Start() {
		...
		theSeries.pointValues = theGraph.GenRandomY(numPoints[0],theGraph.xAxisMinValue,theGraph.xAxisMaxValue,theGraph.yAxisMinValue,theGraph.yAxisMaxValue);
		theSeries.seriesName = "Series 1";
	...
	public void OnSelectionChange4() {
		...
		else if (dropVal == "Axes: Auto X") {
			theGraph.axesType = WMG_Axis_Graph.axesTypes.AUTO_ORIGIN_X;
		}
		else if (dropVal == "Axes: Auto Y") {
			theGraph.axesType = WMG_Axis_Graph.axesTypes.AUTO_ORIGIN_Y;
		}
	}
	...
	public void OnNewDataClick() {
		if (!isAutomaticAnimating) {
			isAutomaticAnimating = true;
			for (int i = 0; i < type.Count; i++) {
				if (type[i] == 5) { // Random-XY
					theGraph.lineSeries[i].GetComponent<WMG_Series>().pointValues = theGraph.GenRandomXY(numPoints[currentSeries],theGraph.xAxisMinValue,theGraph.xAxisMaxValue,theGraph.yAxisMinValue,theGraph.yAxisMaxValue);
				}
				if (type[i] == 6) { // Random-Y
					theGraph.lineSeries[i].GetComponent<WMG_Series>().pointValues = theGraph.GenRandomY(numPoints[currentSeries],theGraph.xAxisMinValue,theGraph.xAxisMaxValue,theGraph.yAxisMinValue,theGraph.yAxisMaxValue);
				}
			}
			HOTween.To(this.transform, 0, new TweenParms()
		            .Prop("localScale", Vector3.one, false)
					.Delay(theGraph.autoAnimationsDuration)
					.OnComplete(setIsAutoAnimating)
		        );
		}
	}
	
	void setIsAutoAnimating() {
		isAutomaticAnimating = false;
	}
	...
	public void OnAnimationPlayClick() {
		...
		if (!isAnimating) {
			...
			if (!(dropVal == "Bar: All" || dropVal == "Bar: Series" || dropVal == "Bar: Point")) {
				HOTween.To(this.transform, 0, new TweenParms()
		            .Prop("localScale", Vector3.one, false)
					.Delay(animationDuration)
					.OnComplete(revertPivots)
		        );
			}
		}
	...
	public void OnAddSeriesClick() {
		...
		addSeries.seriesName = "Series " + theGraph.lineSeries.Count;
		theGraph.addDropdownItem(seriesSelectionDrop, addSeries.seriesName);
		numPoints.Add(7);
		a.Add(0);
		b.Add(0);
		c.Add(0);
		...
		theGraph.setDropdownSelection(seriesSelectionDrop, addSeries.seriesName);
		centerLegendEntries();
	}
